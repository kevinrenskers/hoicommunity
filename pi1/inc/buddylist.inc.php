...
if ($this->piVars['uid'] && $this->piVars['action'] == 'add') {
	// Insert new buddy, only if he is not already on the list
	$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid','tx_hoicommunity_buddylist', 'cruser_id='.intval($this->fe_user->uid).' AND fe_user_uid='.intval($this->piVars['uid']), '', '');
...
if ($this->piVars['uid'] && $this->piVars['action'] == 'delete') {
	// Delete buddy
	$GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_hoicommunity_buddylist', 'cruser_id='.intval($this->fe_user->uid).' AND fe_user_uid='.intval($this->piVars['uid']));
}

// View the buddylist
$select =	'fe_users.username AS username,
					 fe_users.uid AS uid';
$from = 	'tx_hoicommunity_buddylist, fe_users';
$where =	'tx_hoicommunity_buddylist.cruser_id = '.intval($this->fe_user->uid).' AND
					 tx_hoicommunity_buddylist.fe_user_uid = fe_users.uid AND
					 fe_users.disable=0 AND fe_users.deleted=0';
$sort = 	'fe_users.username';

$template['total'] = $this->cObj->getSubpart($this->cObj->fileResource($this->conf['resource.']['template.']['buddylist']),'###BUDDYLIST###');
$template['buddy'] = $this->cObj->getSubpart($template['total'],'###BUDDY###');

$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery($select, $from, $where, '', $sort);
while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
	$markerArray['###USERNAME###'] = htmlspecialchars($row['username']);
	$markerArray['###ROWID###'] = 'tx-hoicommunity-pi1-buddylist-uid'.$row['uid'];
	...
	$markerArray['###ACTIONS###'] =  '<img class="clickable" src="'.$GLOBALS['TSFE']->tmpl->getFileName($this->conf['resource.']['icon.']['messageWrite']).'" alt="'.$this->pi_getLL('sendMessage').'" title="'.$this->pi_getLL('sendMessage').'" onclick="'.$url.'" />'
																	.' <img class="clickable" src="'.$GLOBALS['TSFE']->tmpl->getFileName($this->conf['resource.']['icon.']['buddyDelete']).'" alt="'.$this->pi_getLL('deleteBuddy').'" title="'.$this->pi_getLL('deleteBuddy').'" onclick="deleteBuddy('.$row['uid'].')" />';
	...
	$buddy .= $this->cObj->substituteMarkerArrayCached($template['buddy'], $markerArray, array(), $wrappedSubpartArray);
}
