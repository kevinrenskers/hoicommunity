function validateEditProfile(frm) {
	// Basicly we just check if first name, last name and emailaddress are filled in, and if emailaddress is valid
  var value = '';
  var errFlag = new Array();
  var _qfGroups = {};
  _qfMsg = '';

  value = frm.elements['tx_hoicommunity_pi1[first_name]'].value;
  if (value == '' && !errFlag['tx_hoicommunity_pi1[first_name]']) {
    _qfMsg = _qfMsg + '\n - ' + enterFirstName;
  }

  value = frm.elements['tx_hoicommunity_pi1[last_name]'].value;
  if (value == '' && !errFlag['tx_hoicommunity_pi1[last_name]']) {
    _qfMsg = _qfMsg + '\n - ' + enterLastName;
  }

  value = frm.elements['tx_hoicommunity_pi1[email]'].value;
  if (value == '' && !errFlag['tx_hoicommunity_pi1[email]']) {
    _qfMsg = _qfMsg + '\n - ' + enterEmail;
  }

  value = frm.elements['tx_hoicommunity_pi1[email]'].value;
  var regex = /^((\"[^\"\f\n\r\t\v\b]+\")|([\w\!\#\$\%\&'\*\+\-\~\/\^\`\|\{\}]+(\.[\w\!\#\$\%\&'\*\+\-\~\/\^\`\|\{\}]+)*))@((\[(((25[0-5])|(2[0-4][0-9])|([0-1]?[0-9]?[0-9]))\.((25[0-5])|(2[0-4][0-9])|([0-1]?[0-9]?[0-9]))\.((25[0-5])|(2[0-4][0-9])|([0-1]?[0-9]?[0-9]))\.((25[0-5])|(2[0-4][0-9])|([0-1]?[0-9]?[0-9])))\])|(((25[0-5])|(2[0-4][0-9])|([0-1]?[0-9]?[0-9]))\.((25[0-5])|(2[0-4][0-9])|([0-1]?[0-9]?[0-9]))\.((25[0-5])|(2[0-4][0-9])|([0-1]?[0-9]?[0-9]))\.((25[0-5])|(2[0-4][0-9])|([0-1]?[0-9]?[0-9])))|((([A-Za-z0-9\-])+\.)+[A-Za-z\-]+))$/;
  if (value != '' && !regex.test(value) && !errFlag['tx_hoicommunity_pi1[email]']) {
    _qfMsg = _qfMsg + '\n - ' + wrongEmail;
  }

  if (_qfMsg != '') {
    _qfMsg = formIncorrect + _qfMsg;
    alert(_qfMsg);
    return false;
  }
...
function validateSendMessage(frm) {
  var value = '';
  var errFlag = new Array();
  var _qfGroups = {};
  _qfMsg = '';

  value = frm.elements['tx_hoicommunity_pi1[subject]'].value;
  if (value == '' && !errFlag['tx_hoicommunity_pi1[subject]']) {
    _qfMsg = _qfMsg + '\n - ' + enterSubject;
  }

  value = frm.elements['tx_hoicommunity_pi1[body]'].value;
  if (value == '' && !errFlag['tx_hoicommunity_pi1[body]']) {
    _qfMsg = _qfMsg + '\n - ' + enterMessage;
  }
...
function validateRegister() {
	// First some basic checks: are username, password, first name, last name and emailaddress filled in and is emailaddress valid?
  var frm = $('tx-hoicommunity-pi1-register-formid');
  ...
  value = frm.elements['tx_hoicommunity_pi1[username]'].value;
  if (value == '' && !errFlag['tx_hoicommunity_pi1[username]']) {
    _qfMsg = _qfMsg + '\n - ' + enterNickname;
  }

  value = frm.elements['tx_hoicommunity_pi1[password]'].value;
  if (value == '' && !errFlag['tx_hoicommunity_pi1[password]']) {
    _qfMsg = _qfMsg + '\n - ' + enterPassword;
  }
  ...
  if (_qfMsg != '') {
	...
	$('tx-hoicommunity-pi1-register-submit').disabled=true;
	var oldvalue = $('tx-hoicommunity-pi1-register-submit').value;
	$('tx-hoicommunity-pi1-register-submit').value=pleaseWait;

	// Now we must see if username and/or emailaddress are already taken!
	var pars = 'cmd=registerCheck&username=' + frm.elements['tx_hoicommunity_pi1[username]'].value + '&email=' + frm.elements['tx_hoicommunity_pi1[email]'].value;
	var myAjax = new Ajax.Request(pathToAjax, {
		method: 'post',
		parameters: pars,
		onComplete: function(originalRequest) {
			if (originalRequest.responseText) {
				...
				$('tx-hoicommunity-pi1-register-submit').disabled=false;
				$('tx-hoicommunity-pi1-register-submit').value=oldvalue;
				return false;
			} else {
				frm.submit();
			}
...
function deleteMessage(messageId) {
	// Now we must see if username and/or emailaddress are already taken!
	var pars = 'cmd=deleteMessage&messageId=' + messageId;
	var myAjax = new Ajax.Request(pathToAjax, {
		method: 'post',
		parameters: pars,
		onComplete: function(originalRequest) {
...
function deleteBuddy(buddyId) {
	// Now we must see if username and/or emailaddress are already taken!
	var pars = 'cmd=deleteBuddy&buddyId=' + buddyId;
	var myAjax = new Ajax.Request(pathToAjax, {
		method: 'post',
		parameters: pars,
		onComplete: function(originalRequest) {
...
function deleteIgnoredUser(userId) {
	// Now we must see if username and/or emailaddress are already taken!
	var pars = 'cmd=deleteIgnoredUser&userId=' + userId;
	var myAjax = new Ajax.Request(pathToAjax, {
		method: 'post',
		parameters: pars,
		onComplete: function(originalRequest) {
...
// Global status handlers for AJAX calls
...
// Below this is all for the password strength check
...
function IsNotUsername(strWord) {
	if (username == "") {
		username = $F('tx-hoicommunity-pi1-register-username');
	}
...
function ClientSideStrongPassword() {
	return (IsNotUsername(ClientSideStrongPassword.arguments[0]) &&
					IsLongEnough(ClientSideStrongPassword.arguments[0], "7") &&
...
function ClientSideMediumPassword() {
	return (IsNotUsername(ClientSideMediumPassword.arguments[0]) &&
					IsLongEnough(ClientSideMediumPassword.arguments[0], "7") &&
...
function evalPwd(Sb) {
	if (ClientSideStrongPassword(Sb,gSimilarityMap,gDictionary)) {
		$('tx-hoicommunity-pi1-register-submit').disabled=false;
		$('tx-hoicommunity-pi1-register-password-status').innerHTML='<img alt="" src="/typo3conf/ext/hoicommunity/res/pwd_4.gif" width="400" height="13" />';
	} else{
		if(ClientSideMediumPassword(Sb,gSimilarityMap,gDictionary)) {
			$('tx-hoicommunity-pi1-register-submit').disabled=false;
			$('tx-hoicommunity-pi1-register-password-status').innerHTML='<img alt="" src="/typo3conf/ext/hoicommunity/res/pwd_3.gif" width="400" height="13" />';
		} else{
			if(ClientSideWeakPassword(Sb,gSimilarityMap,gDictionary)) {
				$('tx-hoicommunity-pi1-register-submit').disabled=true;
				$('tx-hoicommunity-pi1-register-password-status').innerHTML='<img alt="" src="/typo3conf/ext/hoicommunity/res/pwd_2.gif" width="400" height="13" />';
			} else {
				$('tx-hoicommunity-pi1-register-submit').disabled=true;
				$('tx-hoicommunity-pi1-register-password-status').innerHTML='<img alt="" src="/typo3conf/ext/hoicommunity/res/pwd_1.gif" width="400" height="13" />';
			}
		}
	}
}